...
export default function TeenDashboard() {
  ...
  const visibleTasks = useMemo(() => {
    ...
    return ageFiltered.filter((task) => approvedCategories.includes(task.category));
  }, [tasks, auth?.profile?.age, auth?.profile?.approvedCategories]);
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <section className="space-y-4">
          <div>
            ...
            <p className="mt-2 text-text-secondary">Only age-appropriate tasks are shown here. Parent approval is required before applying.</p>
          </div>
          ...
          <div className="space-y-3">
            ...
            {auth?.profile?.parentApproved && !loading && !visibleTasks.length ? (
              <PageState title="No tasks near you yet" description="Check back soon or refresh when new neighborhood tasks appear." />
            ) : null}
          </div>
        </section>
