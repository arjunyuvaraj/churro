        ...
        <section className="space-y-4">
          ...
          {!auth?.profile?.parentApproved ? (
            <div className="space-y-3">
              <PageState
                title="Waiting for parent link"
                description="Your account is pending until a parent accepts the link invitation. Check your Alerts tab for status updates."
              />
            </div>
          ) : null}
          ...
          {auth?.profile?.parentApproved && approvedCategories.length > 0 && (
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-medium transition ${selectedCategoryFilter === null ? 'bg-primary text-white' : 'border border-border bg-white text-text-secondary hover:border-primary'
                  }`}
              >
                All tasks
              </button>
              {approvedCategories.map((category) => (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-sm font-medium transition ${selectedCategoryFilter === category ? 'bg-primary text-white' : 'border border-border bg-white text-text-secondary hover:border-primary'
                    }`}
                >
