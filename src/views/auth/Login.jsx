...
export default function Login() {
  ...
  useEffect(() => {
    ...
    // Route to correct destination based on role + profile state
    navigateToRoleDashboard(auth, navigate);
  }, [auth?.loading, auth?.isAuthenticated, auth?.role, auth?.needsProfileSetup, auth?.profile?.surveyCompleted, auth?.profile?.onboardingComplete, navigate, inviteToken]);
  ...
  async function handlePasswordReset(event) {
    ...
    try {
      await auth.passwordReset(resetEmail);
      setResetSent(true);
    } catch (currentError) {
      setError(getReadableAuthError(currentError));
    } finally {
        ...
        <div className="w-full max-w-[30rem]">
          <div className="mb-8">
            ...
            <p className="mt-2 text-sm text-slate-600">
              {resetMode ? (
                <button type="button" onClick={() => { setResetMode(false); setResetSent(false); setError(''); }} className="font-semibold text-slate-900 underline decoration-orange-200 underline-offset-4 transition hover:decoration-orange-700">
                  Back to login
                </button>
              ) : (
                <>
                  Don't have an account?{' '}
                  <Link to={inviteToken ? `/signup?role=parent&inviteToken=${encodeURIComponent(inviteToken)}` : '/signup'} className="font-semibold text-slate-900 underline decoration-orange-200 underline-offset-4 transition hover:decoration-orange-700">Sign up</Link>
                </>
              )}
            </p>
          </div>
            ...
            {resetMode ? (
              <form onSubmit={handlePasswordReset} className="space-y-4">
                <label className="block">
                  <span className="mb-1.5 block text-sm font-semibold text-slate-900">Email address</span>
                  <input
              ...
              <>
                ...
                <p className="text-center text-sm text-slate-500">
                  Forgot password?{' '}
                  <button type="button" onClick={() => { setResetMode(true); setError(''); }} className="font-semibold text-slate-900 underline decoration-orange-200 underline-offset-4 transition hover:decoration-orange-700">
                    Click here
                  </button>
                </p>
              </>
...
/** Route to correct dashboard based on role and profile completeness */
export function navigateToRoleDashboard(auth, navigate) {
  if (!auth?.role) return;

  if (auth.role === 'teen') {
    navigate(auth.profile?.surveyCompleted ? '/teen' : '/teen/survey', { replace: true });
  } else if (auth.role === 'parent') {
    navigate(auth.profile?.onboardingComplete ? '/parent' : '/parent/onboarding', { replace: true });
  } else {
